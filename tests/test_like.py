...
def test_like_on_post(authorized_client, test_posts):
    res = authorized_client.post("/like/", json={"post_id": test_posts[3].id})
...
def test_like_post_non_exist(authorized_client, test_posts):
    res = authorized_client.post("/like/", json={"post_id": 80000})
    assert res.status_code == 404


def test_like_unauthorized_user(client, test_posts):
    res = client.post("/like/", json={"post_id": test_posts[3].id})
